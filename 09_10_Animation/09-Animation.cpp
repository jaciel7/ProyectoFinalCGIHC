...
bool Start() {
	...
	isla2 = new Model("models/isla7.fbx");
	house = new Model("models/visor.fbx");
	hotel1 = new Model("models/puente.fbx");
	isla3 = new Model("models/isla7.fbx"); 
	mesa = new Model("models/isla7.fbx");
	house2 = new Model("models/visor.fbx");
	isla4 = new Model("models/isla7.fbx"); 

	mesa777 = new Model("models/mesa777.fbx"); 
	city2 = new Model("models/city.fbx"); 
	plano2 = new Model("models/isla8.fbx"); 

	particleModel = new Model("models/snow/snow.fbx");

	character01 = new AnimatedModel("models/IllumModels/KAYA.fbx");
	character02 = new AnimatedModel("models/persona2.fbx"); 
	character03 = new AnimatedModel("models/persona3.fbx"); 
	character04 = new AnimatedModel("models/persona4.fbx"); 
	mar = new AnimatedModel("models/animabarco1.fbx"); 
...
bool Update() {
...

	{
		...
		//dibujamos la segunda isla 
		model = glm::mat4(4.0f);
		model = glm::translate(model, glm::vec3(-12.5f, 0.0f, -1.0f)); // translate it down so it's at the center of the scene
		model = glm::rotate(model, glm::radians(0.0f), glm::vec3(1.0f, 0.0f, 0.0f));
		model = glm::scale(model, glm::vec3(0.05f, 0.05f, 0.05f));	// it's a bit too big for our scene, so scale it down
		staticShader->setMat4("model", model);

		//isla2->Draw(*staticShader);
		...
		//isla3->Draw(*staticShader);

		//dibujamos las mesas 
		model = glm::mat4(6.0f);
		model = glm::translate(model, glm::vec3(-20.0f, 0.0f, 10.0f)); // translate it down so it's at the center of the scene
		model = glm::rotate(model, glm::radians(0.0f), glm::vec3(1.0f, 0.0f, 0.0f));
		model = glm::scale(model, glm::vec3(0.05f, 0.05f, 0.05f));	// it's a bit too big for our scene, so scale it down
		staticShader->setMat4("model", model);
		...
		// dibujamos otro visor
		model = glm::mat4(7.0f);
		model = glm::translate(model, glm::vec3(-10.0f, 0.0f, -7.0f)); // translate it down so it's at the center of the scene
		model = glm::rotate(model, glm::radians(-90.0f), glm::vec3(1.0f, 0.0f, 0.0f));
		model = glm::scale(model, glm::vec3(0.5f, 0.5f, 0.5f));	// it's a bit too big for our scene, so scale it down
		staticShader->setMat4("model", model);
		...
		// dibujamos mas modelos 
		model = glm::mat4(8.0f);
		model = glm::translate(model, glm::vec3(9.0f, 0.0f, -7.0f)); // translate it down so it's at the center of the scene
		model = glm::rotate(model, glm::radians(0.0f), glm::vec3(1.0f, 0.0f, 0.0f));
		model = glm::scale(model, glm::vec3(0.05f, 0.05f, 0.05f));	// it's a bit too big for our scene, so scale it down
		staticShader->setMat4("model", model);

		//isla4->Draw(*staticShader);
		...
		//mesa777->Draw(*staticShader);


		model = glm::mat4(10.0f);
		model = glm::translate(model, glm::vec3(9.0f, 0.0f, -7.0f)); // translate it down so it's at the center of the scene
		model = glm::rotate(model, glm::radians(-90.0f), glm::vec3(1.0f, 0.0f, 0.0f));
		model = glm::scale(model, glm::vec3(1.0f, 1.0f, 1.0f));	// it's a bit too big for our scene, so scale it down
		staticShader->setMat4("model", model);

		//city2->Draw(*staticShader);



		//medidas de la carpa
		model = glm::mat4(11.0f);
		model = glm::translate(model, glm::vec3(9.0f, 0.0f, -7.0f)); // translate it down so it's at the center of the scene
		model = glm::rotate(model, glm::radians(-90.0f), glm::vec3(1.0f, 0.0f, 0.0f));
		model = glm::scale(model, glm::vec3(10.0f, 20.0f, 0.7f));	// it's a bit too big for our scene, so scale it down
		staticShader->setMat4("model", model);

		//plano2->Draw(*staticShader);



		//medidas para la playa 
		model = glm::mat4(11.0f);
	...
	*/
	...
	
	{
		...
		model = glm::translate(model, glm::vec3(-5.0f, 1.9f, -1.0f)); // translate it down so it's at the center of the scene
		model = glm::rotate(model, glm::radians(rotateCharacter), glm::vec3(0.0, 1.0f, 0.0f));
...

	{
		...
		// Aplicamos transformaciones del modelo
		glm::mat4 model = glm::mat4(2.0f);
		model = glm::translate(model, glm::vec3(-3.0f, 1.9f, -1.0f)); // translate it down so it's at the center of the scene
		model = glm::rotate(model, glm::radians(rotateCharacter), glm::vec3(0.0, 1.0f, 0.0f));
...

	{
		...
		model = glm::translate(model, glm::vec3(-7.0f, 1.9f, -1.0f)); // translate it down so it's at the center of the scene
		model = glm::rotate(model, glm::radians(rotateCharacter), glm::vec3(0.0, 1.0f, 0.0f));
		model = glm::scale(model, glm::vec3(0.0005f, 0.0005f, 0.0005f));	// it's a bit too big for our scene, so scale it down
...

	{
		...
		// Aplicamos transformaciones del modelo
		glm::mat4 model = glm::mat4(3.0f);
		model = glm::translate(model, glm::vec3(-10.0f, 1.7f, -1.0f)); // translate it down so it's at the center of the scene
		model = glm::rotate(model, glm::radians(rotateCharacter), glm::vec3(0.0, 1.0f, 0.0f));
		model = glm::scale(model, glm::vec3(0.0005f, 0.0005f, 0.0005f));	// it's a bit too big for our scene, so scale it down
...

	{
		...
		dynamicShader->setMat4("gBones", MAX_RIGGING_BONES, mar->gBones);

		// Dibujamos el modelo
		//mar->Draw(*dynamicShader);
